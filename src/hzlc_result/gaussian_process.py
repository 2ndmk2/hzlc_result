...
def run_inference(model, num_samples, num_warmup, num_chains, thinning, rng_key, X, Y):
    start = time.time()
...
def make_args_for_mcmc(num_samples, num_warmup, thinning):


def main(num_samples, num_warmup, thinning):
    X, Y, X_test = get_data(N=args.num_data)

    # do inference
    rng_key, rng_key_predict = random.split(random.PRNGKey(0))
    samples = run_inference(model, num_samples, num_warmup, num_chains, thinning, rng_key, X, Y)
